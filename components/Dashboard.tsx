...
const Dashboard: React.FC = () => {
  ...
  const visibleSchedule = useMemo(() => user.role === 'vocal_teacher' 
    ? schedule.filter(c => c.workshop === 'Técnica Vocal')
    : schedule, [schedule, user.role]);
  ...
  const scheduleByDay = useMemo(() => daysOfWeekOrder.reduce((acc, day) => {
    acc[day] = visibleSchedule
      .filter(c => c.dayOfWeek === day)
      .sort((a, b) => a.time.localeCompare(b.time));
    return acc;
  }, {} as Record<string, ScheduledClass[]>), [visibleSchedule]);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <p className="text-center text-slate-500 dark:text-slate-400">Carregando horários...</p>
      ) : (
